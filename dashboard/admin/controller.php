...
<?php
  ...
  if (isset($_GET['sortfields'])):
      foreach ($_POST['node'] as $k => $v):
          ...
          $db->update(Core::fTable, $data, "id=" . (int)$v);
      endforeach;
  endif;
  ...
  if (isset($_POST['deleteField'])):
      ...
      $db->delete(Core::fTable, "id='" . $id . "'");
      $title = sanitize($_POST['title']);

      print ($db->affected()) ? Filter::msgOk('Field <strong>' . $title . '</strong> deleted successfully!') : Filter::msgAlert('<span>Alert!</span>Nothing to process.');
  endif;
